...
class BuildTool:
    ...
    def __init__(self):
        ...
        self.app_name = "StardewValleyTranslationTool"
    ...
    def build_onefile(self, output_dir, exe_name=None):
        """构建单文件版本"""
        self.print_header("构建单文件版本")
        
        # Nuitka命令 - 确保单文件模式并禁用控制台窗口
        cmd = [
            'nuitka',
            '--standalone',
            '--onefile',
        ...
        # 如果指定了exe文件名，添加--output-filename参数
        if exe_name:
            cmd.extend(['--output-filename', exe_name])
        ...
        try:
            ...
            if return_code == 0:
                ...
                # 检查生成的文件
                exe_file = output_dir / (exe_name if exe_name else 'main.exe')
                ...
                if exe_file.exists():
                    ...
                    for location in other_locations:
                        if location.exists():
                            ...
                            target_file = output_dir / (exe_name if exe_name else 'main.exe')
                            import shutil
    ...
    def run(self):
        ...
        exe_name = f"{self.app_name}_v{self.app_version}.exe"
        verify_path = output_dir / exe_name
        ...
        # 3. 构建
        if not self.build_onefile(output_dir, exe_name):
            return False
