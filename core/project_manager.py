...
class ProjectManager(QObject):
    ...
    def _check_project_structure(self):
        ...
        for folder in self.project_structure.keys():
            ...
            if not os.path.exists(folder_path):
                ...
                os.makedirs(folder_path, exist_ok=True)
                signal_bus.log_message.emit("INFO", "创建缺失的文件夹", {
                    "folder_name": folder,
                    "folder_path": folder_path
                })

        if missing_folders:
            signal_bus.log_message.emit("SUCCESS", f"已创建 {len(missing_folders)} 个缺失的文件夹", {
    ...
    def get_folder_path(self, folder_type: str) -> str:
        ...
        try:
            ...
            signal_bus.log_message.emit("SUCCESS", f"获取 {folder_type} 文件夹📁 路径成功", {
