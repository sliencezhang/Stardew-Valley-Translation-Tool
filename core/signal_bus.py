...
class SignalBus(QObject):
    ...
    # progress dialog
    translationDialogClosed = Signal()  # 翻译进度对话框关闭信号
    ...
    # tab_one_click_update
    startOneClickUpdate = Signal(dict)
    showNameDetectionDialog = Signal(list, object)  # 检测结果列表, 项目管理器
    nameDetectionCompleted = Signal(dict)  # 人名地名检测完成

    # update system
    update_available = Signal(dict)  # 有新版本可用时发送更新信息
