...
class QualityChecker:
    ...
    def _check_file(self, en_data: Dict, zh_data: Dict, filename: str) -> List[Dict]:
        """检查单个文件"""
        issues = []
        ...
        for key, en_value in en_data.items():
            ...
            if self.enable_variable_check and (var_issue := self._check_variables(en_value, zh_value)):
                issues.append(self._build_issue(key, en_value, zh_value, filename, '变量不匹配', var_issue))
            elif self.enable_untranslated_check and self._is_untranslated(en_value, zh_value):
                issues.append(self._build_issue(key, en_value, zh_value, filename, '未翻译',{'新翻译': ""}))
            elif self.enable_english_check and (eng_issue := self._check_english_content(zh_value)):
                issues.append(self._build_issue(key, en_value, zh_value, filename, '中英文夹杂', eng_issue))
    ...
    def _build_issue(key: str, en_text: str, zh_text: str, filename: str,
                     issue_type: str, details: Dict) -> Dict:
        """构建问题字典"""
        return {
            '键': key,
            '英文': en_text,
