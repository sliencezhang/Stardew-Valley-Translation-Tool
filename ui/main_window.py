...
class StardewTranslationTool(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 添加标签页
        self.tab_widget.addTab(self.one_click_update_tab, "🔄 一键更新")
    ...
    def apply_quality_fixes(self, params):
        ...
        try:
            ...
            for key, fix_data in fixes.items():
                ...
                original_key = fix_data.get('键', key)  # 如果有原始键则使用，否则使用哈希键
                mod_name = fix_data.get('mod_name', '')
                filename = fix_data.get('filename', '')
                source_file = fix_data.get('原始文件', '')  # 兼容质量检查标签页的格式

                # 检查是否是独立质量检查窗口的请求（有mod_name和filename）
                if mod_name and filename:
                    ...
                    if not os.path.exists(i18n_path):
                        signal_bus.log_message.emit("WARNING", f"mod {mod_name} 的i18n文件夹不存在", {})
                        continue
                    ...
                    # 通过哈希键在mod_mapping中查找原始键
                    lookup_key = original_key
                    # 从translation_executor获取mod_mapping
            ...
            for target_file, updates in file_updates.items():
                if os.path.exists(target_file):
                    signal_bus.log_message.emit("INFO", f"批量更新文件: {target_file}", {})
                    ...
                    if isinstance(data, dict):
                        ...
                        signal_bus.log_message.emit("INFO", f"批量更新完成: {target_file}, 更新了 {len(updates)} 项",
                                                    {})
            ...
            signal_bus.log_message.emit("SUCCESS",
                                        f"💾 已成功应用 {applied_count} 个修复，修改了 {len(modified_files)} 个文件", {})
    ...
    def start_one_click_update(self, params):
        ...
        self.run_worker_in_thread("one_click_update", params, "一键更新")
    ...
    def show_name_detection_dialog(self, results_list, project_manager):
        ...
        try:
            ...
            signal_bus.log_message.emit("ERROR", f"显示人名地名检测对话框失败: {str(e)}", {})
            # 如果出错，也设置结果为True以继续执行
            if hasattr(self, 'translation_executor') and hasattr(self.translation_executor, '_current_processor'):
                processor = self.translation_executor._current_processor
                if hasattr(processor, 'set_name_detection_result'):
    ...
    def on_name_detection_completed(self, result):
        """人名地名检测完成处理"""
        try:
            detection_results = result.get('检测结果', [])
            if detection_results:
                ...
                signal_bus.log_message.emit("INFO", "人名地名检测结果已显示", {})
            else:
                signal_bus.log_message.emit("INFO", "未检测到人名地名，继续执行", {})
            ...
            signal_bus.log_message.emit("ERROR", f"处理人名地名检测结果失败: {str(e)}", {})
            # 即使出错也要继续执行
            if hasattr(self, 'translation_executor') and hasattr(self.translation_executor, '_current_processor'):
                processor = self.translation_executor._current_processor
                if hasattr(processor, '_user_processed'):
    ...
    def on_update_available(self, update_info):
        ...
        try:
            ...
            signal_bus.log_message.emit("ERROR", f"显示更新对话框失败: {str(e)}", {})
        ...
        try:
            ...
            if (hasattr(self.translation_executor, '_current_processor') and 
                ...
                if translation_results:
                    ...
                    signal_bus.log_message.emit("INFO", f"更新了独立质量检查窗口的 {len(translation_results)} 项翻译", {})
                    
        except Exception as e:
            signal_bus.log_message.emit("ERROR", f"更新独立质量检查窗口失败: {str(e)}", {})
    ...
    def on_one_click_update_complete(self, result):
        ...
        success = result.get('成功', False)
        if success:
            signal_bus.log_message.emit("SUCCESS", result.get('消息', ''), result.get('输出文件夹', ''))
            # 保存处理器引用
            if hasattr(self.translation_executor, '_current_processor'):
            ...
            signal_bus.log_message.emit("ERROR", result.get('消息', '') , {})
