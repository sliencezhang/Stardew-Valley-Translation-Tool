...
class TabQualityCheck(QWidget):
    ...
    def run_quality_check(self):
        ...
        signal_bus.log_message.emit("INFO", f"运行质量检查: {params}", {})
        signal_bus.runQualityCheck.emit(params)
                ...
                if new_text and new_text != current_translation:
                    ...
                    for fix_key, fix_data in self.quality_fixes.items():
                        if fix_data.get('键') == key:
                            ...
                            signal_bus.log_message.emit("DEBUG", f"更新键 {key} 的翻译为: {new_text[:50]}...", {})
                            break
